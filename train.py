...
flags.DEFINE_integer('shuffle_seq_length', 3,
                     'Number of images for each input group.')
flags.DEFINE_string('pro2_calibration_file', '', 'Json calibration file for the Insta360 Pro 2')
...
flags.DEFINE_integer('max_steps', 10000000,
                     'Maximum number of training steps.')
flags.DEFINE_integer('summary_freq', 5, 'Logging frequency.')
flags.DEFINE_integer('save_latest_freq', 2000,
                     'Frequency with which to save the model (overwrites previous model).')
...
flags.DEFINE_boolean('coord_net', False,
                     'Whether to append CoordNet during convolution.')
flags.DEFINE_boolean('transform_inverse_reg', False,
                     'Whether to train with transform-inverse regularization.')
...
flags.DEFINE_integer('num_psv_planes', 32,
...
flags.DEFINE_boolean('spherical_attention', False,
                     'Calculate loss with a spherically aware weight map.')
...
flags.DEFINE_boolean(
    'dry_run', False, 'Dry run to save images without inference. [src, ref, tgt images + formatted psv]')
...
flags.DEFINE_boolean('mixed_precision', False,
                     "Enable mixed precision training.")
flags.DEFINE_string('supervision', 'tgt',
                    "Images to supervise on. [tgt, ref, src, hrestgt] concatenated with _")
flags.DEFINE_float('rot_factor', 1.0,
                   'for experiments with rotation jittering range, n x 0.03 radians')
flags.DEFINE_float(
    'tr_factor', 1.0, 'for experiments with translation jittering range, n x 0.01')

flags.DEFINE_boolean('gcn', False, 'Train with gcn.')
flags.DEFINE_integer(
    'subdiv', 7, 'subdivision level for the spherical mesh we want to operate on.')
...
def main(_):
    ...
    if FLAGS.input_type == 'PP':
        assert('hrestgt' not in FLAGS.supervision)
    ...
    if 'PP' in FLAGS.input_type:
        train_batch['interp_pose'] = interpolate_pose(
            train_batch["ref_pose"], train_batch["src_pose"])
    ...
    if FLAGS.input_type != 'PRO2':
        ...
        train_batch['ref_pose_inv'] = tf.matrix_inverse(
            train_batch['ref_pose'], name='ref_pose_inv')
    train_batch['jitter_pose'] = tf.identity(tf_random_rotation(
        FLAGS.rot_factor, FLAGS.tr_factor), name='jitter_pose')
    ...
    if FLAGS.gcn:
        ...
        train_batch['p2v'] = tf.dtypes.cast(tf.identity(
            pix2vertex_lookup, name='p2v'), dtype=tf.float32)

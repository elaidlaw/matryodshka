...
class MSI(object):
    ...
    def infer_msi(self,
        ...
        # Preprocessing
        if FLAGS.input_type != 'PRO2':
            batch_size, img_height, img_width, _ = raw_src_image.get_shape().as_list()
        else:
            batch_size, cams, img_height, img_width, _ = raw_src_image.get_shape().as_list()
    ...
    '''
        if which_color_pred == 'blend_psv':
            if FLAGS.transform_inverse_reg:
                if not FLAGS.jitter:
                    msi_pred = msi_net(
                        net_input, num_msi_planes * 2, ngf=FLAGS.ngf, reuse_weights=False)
                else:
                    msi_pred = msi_net(
                        net_input, num_msi_planes * 2, ngf=FLAGS.ngf, reuse_weights=True)
            else:
                print(net_input)
                msi_pred = msi_net(
                    net_input, num_msi_planes * 2, ngf=FLAGS.ngf)
            if FLAGS.net_only:
                # for model export
                return None
            with tf.name_scope('layer_prediction'):
                ...
                alphas = (
                    msi_pred[:, :, :, num_msi_planes:num_msi_planes * 2] + 1.) / 2.

                # Assemble into an msi (rgba_layers)
                for i in range(num_msi_planes):
                    fg_rgb = net_input[:, :, :, i * 3:(1 + i) * 3]
                    bg_rgb = net_input[:, :, :, (num_msi_planes + i)
                                       * 3:(num_msi_planes + 1 + i) * 3]
                    ...
                    if i == 0:
                        ...
                        rgba_layers = tf.concat(
                            [rgba_layers, curr_rgba], axis=3)
                rgba_layers = tf.reshape(
                    rgba_layers, [batch_size, img_height, img_width, num_msi_planes, 4])

                # Upsampled rgba layers
                if 'hrestgt' in FLAGS.supervision:
                    upsampled_blend_weights = tf.image.resize(blend_weights, [
                                                              FLAGS.hres_height, FLAGS.hres_width], align_corners=True, method=tf.image.ResizeMethod.BILINEAR)
                    upsampled_alphas = tf.image.resize(alphas, [
                                                       FLAGS.hres_height, FLAGS.hres_width], align_corners=True, method=tf.image.ResizeMethod.BILINEAR)
                    # Assemble into an msi (rgba_layers)
                    for i in range(num_msi_planes):
                        ufg_rgb = hres_net_input[:, :, :, i * 3:(1 + i) * 3]
                        ubg_rgb = hres_net_input[:, :,
                                                 :, (32 + i) * 3:(33 + i) * 3]
                        ucurr_alpha = tf.expand_dims(
                            upsampled_alphas[:, :, :, i], -1)
                        uw = tf.expand_dims(
                            upsampled_blend_weights[:, :, :, i], -1)
                        ucurr_rgb = uw * ufg_rgb + (1 - uw) * ubg_rgb
                        ucurr_rgba = tf.concat(
                            [ucurr_rgb, ucurr_alpha], axis=3)
                        if i == 0:
                            urgba_layers = ucurr_rgba
                        else:
                            urgba_layers = tf.concat(
                                [urgba_layers, ucurr_rgba], axis=3)
                    urgba_layers = tf.reshape(
                        urgba_layers, [batch_size, hres_img_height, hres_img_width, num_msi_planes, 4])
        elif which_color_pred == 'blend_bg':
            if FLAGS.transform_inverse_reg:
                if not FLAGS.jitter:
                    msi_pred = msi_net(
                        net_input, 3 + num_msi_planes * 2, ngf=FLAGS.ngf, reuse_weights=False)
                else:
                    msi_pred = msi_net(
                        net_input, 3 + num_msi_planes * 2, ngf=FLAGS.ngf, reuse_weights=True)
            else:
                msi_pred = msi_net(
                    net_input, 3 + num_msi_planes * 2, ngf=FLAGS.ngf)
            if FLAGS.net_only:
                return None
            with tf.name_scope('layer_prediction'):
                ...
                if 'hrestgt' in FLAGS.supervision:
                    ...
                    upsampled_alphas = tf.image.resize(alphas, [
                                                       FLAGS.hres_height, FLAGS.hres_width], align_corners=True, method=tf.image.ResizeMethod.BILINEAR)
                    ubg_rgb = tf.image.resize(bg_rgb, [
                                              FLAGS.hres_height, FLAGS.hres_width], align_corners=True, method=tf.image.ResizeMethod.BILINEAR)
                    # Assemble into an msi (rgba_layers)
                    for i in range(num_msi_planes):
                        ufg_rgb = hres_net_input[:, :, :, i * 3:(1 + i) * 3]
            ...
            if FLAGS.transform_inverse_reg:
                if not FLAGS.jitter:
                    msi_pred = msi_net(
                        net_input, 3 + num_msi_planes * 3, ngf=FLAGS.ngf, reuse_weights=False)
                else:
                    msi_pred = msi_net(
                        net_input, 3 + num_msi_planes * 3, ngf=FLAGS.ngf, reuse_weights=True)
            else:
                msi_pred = msi_net(
                    net_input, 3 + num_msi_planes * 3, ngf=FLAGS.ngf)
            if FLAGS.net_only:
                return None
            with tf.name_scope('layer_prediction'):
                # Rescale blend_weights to (0, 1)
                blend_weights = (msi_pred[:, :, :, :num_msi_planes] + 1.) / 2.
                bg_blend_weights = (
                    msi_pred[:, :, :, num_msi_planes * 2:num_msi_planes * 3] + 1.) / 2.
                # Rescale alphas to (0, 1)
                alphas = (
                    msi_pred[:, :, :, num_msi_planes:num_msi_planes * 2] + 1.) / 2.
                pred_bg = msi_pred[:, :, :, -3:]
                ...
                for i in range(num_msi_planes):
                    ...
                    fg_rgb = net_input[:, :, :, i * 3:(1 + i) * 3]
                    bg_rgb = net_input[:, :, :,
                                       (num_msi_planes+i) * 3:(num_msi_planes+1+i) * 3]
                    w = tf.expand_dims(blend_weights[:, :, :, i], -1)
                    curr_rgb = w * fg_rgb + (1 - w) * bg_rgb
            ...
            if FLAGS.transform_inverse_reg:
                if not FLAGS.jitter:
                    msi_pred = msi_net(
                        net_input, num_msi_planes, ngf=FLAGS.ngf, reuse_weights=False)
                else:
                    msi_pred = msi_net(
                        net_input, num_msi_planes, ngf=FLAGS.ngf, reuse_weights=True)
            else:
    ...
    def infer_gcn_msi(self,
      ...
      '''
        if which_color_pred == 'blend_psv':
            msi_pred = msi_net(net_input, num_msi_planes *
                               2, support, ngf=FLAGS.ngf)
    ...
    def msi_render_equirect_depth(self, rgba_layers, tgt_pose_rt, tgt_pos, planes, intrinsics):
        ...
        proj_images = pj.projective_forward_sphere(
            rgba_layers, intrinsics, tgt_pose_rt, tgt_pos, depths)
    ...
    def msi_render_perspective_view(self, rgba_layers, tgt_pose_rt, tgt_pos, planes, intrinsics,
        ...
        proj_images = pj.projective_forward_sphere_to_perspective(
            rgba_layers, intrinsics, tgt_pose_rt, tgt_pos, depths, viewing_window, psp_height, psp_width)
    ...
    def msi_render_ods_view(self, rgba_layers, order, jitter_pose, tgt_pos, planes, intrinsics):
        ...
        depths = tf.tile(depths, [1, batch_size])
        rgba_layers = tf.transpose(rgba_layers, [3, 0, 1, 2, 4])
        proj_images = pj.projective_forward_ods(
            rgba_layers, order, intrinsics, jitter_pose, tgt_pos, depths)
    ...
    def build_train_graph(self,
        ...
        with tf.name_scope('synthesis'):
            if FLAGS.input_type == 'ODS' or FLAGS.input_type == 'PRO2':
                if 'tgt' in FLAGS.supervision:
                    output_image = self.msi_render_equirect_view(rgba_layers, tf.expand_dims(
                        tf.eye(4), axis=0), tgt_pose, msi_planes, intrinsics)
                    output_depth_image = self.msi_render_equirect_depth(
                        rgba_layers, tf.expand_dims(tf.eye(4), axis=0), tgt_pose, msi_planes, intrinsics)
                if 'hrestgt' in FLAGS.supervision:
                    hres_output_image = self.msi_render_equirect_view(hrgba_layers, tf.expand_dims(
                        tf.eye(4), axis=0), tgt_pose, msi_planes, intrinsics)
                    hres_output_depth_image = self.msi_render_equirect_depth(
                        hrgba_layers, tf.expand_dims(tf.eye(4), axis=0), tgt_pose, msi_planes, intrinsics)
                if 'src' in FLAGS.supervision:
                    src_output_image = self.msi_render_ods_view(
                        rgba_layers, -1, tf.expand_dims(tf.eye(4), axis=0), tgt_pose, msi_planes, intrinsics)
                if 'ref' in FLAGS.supervision:
                    ref_output_image = self.msi_render_ods_view(rgba_layers, 1, tf.expand_dims(
                        tf.eye(4), axis=0), tgt_pose, msi_planes, intrinsics)

                if FLAGS.transform_inverse_reg:
                    jitter_pose = tf.get_default_graph().get_tensor_by_name("jitter_pose:0")
                    if 'tgt' in FLAGS.supervision:
                        jitter_output_image = self.msi_render_equirect_view(
                            rgba_layers_jitter, jitter_pose, tgt_pose, msi_planes, intrinsics)
                        jitter_output_depth_image = self.msi_render_equirect_depth(
                            rgba_layers_jitter, jitter_pose, tgt_pose, msi_planes, intrinsics)
                    if 'src' in FLAGS.supervision:
                        jsrc_output_image = self.msi_render_ods_view(
                            rgba_layers, -1, jitter_pose, tgt_pose, msi_planes, intrinsics)
                    if 'ref' in FLAGS.supervision:
                        jref_output_image = self.msi_render_ods_view(
                            rgba_layers, 1, jitter_pose, tgt_pose, msi_planes, intrinsics)
                ...
                output_image = self.mpi_render_view(
                    rgba_layers, rel_pose, msi_planes, intrinsics)
                if FLAGS.transform_inverse_reg:
                    ...
                    rel_pose = tf.matmul(tgt_pose, ref_pose_inv)
                    jitter_output_image = self.mpi_render_view(
                        rgba_layers_jitter, rel_pose, msi_planes, intrinsics)
        with tf.name_scope('loss'):

            # loss functions
            metric = elpips.Metric(elpips.elpips_vgg(
                batch_size=1), back_prop=True)
            ...
            if which_loss == 'pixel':
                if FLAGS.transform_inverse_reg:
                    if 'tgt' in FLAGS.supervision:
                        # tf.reduce_mean(tf.nn.l2_loss(output_image - tgt_image))
                        reg_reconstruction_loss = get_loss(
                            output_image, tgt_image, 'pixel', FLAGS.spherical_attention)
                        # tf.reduce_mean(tf.nn.l2_loss(jitter_output_image - tgt_image))
                        jitter_reconstruction_loss = get_loss(
                            jitter_output_image, tgt_image, 'pixel', FLAGS.spherical_attention)
                        # tf.reduce_mean(tf.nn.l2_loss(output_image - jitter_output_image))
                        enforcement_loss = get_loss(
                            jitter_output_image, output_image, 'pixel', FLAGS.spherical_attention)
                        total_loss += reg_reconstruction_loss
                        total_loss += 10 * enforcement_loss
                    if 'src' in FLAGS.supervision:
                        # tf.reduce_mean(tf.nn.l2_loss(src_output_image - src_image))
                        total_loss += get_loss(src_output_image, src_image,
                                               'pixel', FLAGS.spherical_attention)
                        # tf.reduce_mean(tf.nn.l2_loss(jsrc_output_image - src_image))
                        total_loss += get_loss(jsrc_output_image,
                                               src_image, 'pixel', FLAGS.spherical_attention)
                    if 'ref' in FLAGS.supervision:
                        # tf.reduce_mean(tf.nn.l2_loss(ref_output_image - ref_image))
                        total_loss += get_loss(ref_output_image, ref_image,
                                               'pixel', FLAGS.spherical_attention)
                        # tf.reduce_mean(tf.nn.l2_loss(jref_output_image - ref_image))
                        total_loss += get_loss(jref_output_image,
                                               ref_image, 'pixel', FLAGS.spherical_attention)
                else:
                    if 'tgt' in FLAGS.supervision:
                        # tf.reduce_mean(tf.nn.l2_loss(output_image - tgt_image))
                        total_loss += get_loss(output_image, tgt_image,
                                               'pixel', FLAGS.spherical_attention)
                    if 'hrestgt' in FLAGS.supervision:
                        # tf.reduce_mean(tf.nn.l2_loss(hres_output_image - hres_tgt_image))
                        total_loss += get_loss(hres_output_image, hres_tgt_image,
                                               'pixel', FLAGS.spherical_attention)
                    if 'src' in FLAGS.supervision:
                        # tf.reduce_mean(tf.nn.l2_loss(src_output_image - src_image))
                        total_loss += 0.0001 * \
                            get_loss(src_output_image, src_image,
                                     'pixel', FLAGS.spherical_attention)
                    if 'ref' in FLAGS.supervision:
                        # tf.reduce_mean(tf.nn.l2_loss(ref_output_image - ref_image))
                        total_loss += 0.0001 * \
                            get_loss(ref_output_image, ref_image,
                                     'pixel', FLAGS.spherical_attention)
            if which_loss == 'elpips':
                if FLAGS.transform_inverse_reg:
                    if 'tgt' in FLAGS.supervision:
                        reg_reconstruction_loss = get_loss(
                            output_image, tgt_image, 'elpips', FLAGS.spherical_attention)
                        jitter_reconstruction_loss = get_loss(
                            jitter_output_image, tgt_image, 'elpips', FLAGS.spherical_attention)
                        enforcement_loss = get_loss(
                            jitter_output_image, output_image, 'elpips', FLAGS.spherical_attention)
                        total_loss += reg_reconstruction_loss
                        total_loss += 10 * enforcement_loss
                    if 'src' in FLAGS.supervision:
                        total_loss += get_loss(src_output_image, src_image,
                                               'elpips', FLAGS.spherical_attention)
                        total_loss += get_loss(jsrc_output_image, src_image,
                                               'elpips', FLAGS.spherical_attention)
                    if 'ref' in FLAGS.supervision:
                        total_loss += get_loss(ref_output_image, ref_image,
                                               'elpips', FLAGS.spherical_attention)
                        total_loss += get_loss(jref_output_image, ref_image,
                                               'elpips', FLAGS.spherical_attention)
                else:
                    if 'tgt' in FLAGS.supervision:
                        total_loss += get_loss(output_image, tgt_image,
        ...
        with tf.name_scope('train_op'):
            ...
            if FLAGS.mixed_precision:
                optim = tf.train.experimental.enable_mixed_precision_graph_rewrite(
                    optim)
            grads_and_vars = optim.compute_gradients(
                total_loss, var_list=train_vars)
        ...
        if FLAGS.transform_inverse_reg and 'tgt' in FLAGS.supervision:
            ...
            tf.summary.scalar('jitter_reconstr_loss',
                              jitter_reconstruction_loss)
        ...
        # Source images
        if FLAGS.input_type == 'PRO2':
            tf.summary.image('src_image', tf.squeeze(tf.slice(raw_src_image, [0, 0, 0, 0, 0], [1, 1, 320, 640, 3]), 1))
        else:
            tf.summary.image('src_image', raw_src_image)
        tf.summary.image('net_input1', self.deprocess_image(tf.slice(net_input, [0, 0, 0, 48], [1, 320, 640, 3])))
        tf.summary.image('net_input2', self.deprocess_image(tf.slice(net_input, [0, 0, 0, 144], [1, 320, 640, 3])))
        # Target image
        ...
        if 'tgt' in FLAGS.supervision:
            tf.summary.image(
                'output_image', self.deprocess_image(output_image))
            if FLAGS.transform_inverse_reg:
                tf.summary.image('jitter_output_image',
                                 self.deprocess_image(jitter_output_image))
        if 'src' in FLAGS.supervision:
            tf.summary.image('src_output_image',
                             self.deprocess_image(src_output_image))
        if 'ref' in FLAGS.supervision:
            tf.summary.image('ref_output_image',
                             self.deprocess_image(ref_output_image))
        ...
        for i in [0, 8, 16, 24, 31]:
            ...
            if FLAGS.transform_inverse_reg:
                ...
                tf.summary.image('jitter_rgb_layer_%d' %
                                 i, self.deprocess_image(jrgb))
                tf.summary.image('jitter_alpha_layer_%d' % i, jalpha)
                tf.summary.image('jitter_rgba_layer_%d' %
                                 i, self.deprocess_image(jrgb * jalpha))
            tf.summary.image('rgb_layer_%d' % i, self.deprocess_image(rgb))
            tf.summary.image('alpha_layer_%d' % i, alpha)
            tf.summary.image('rgba_layer_%d' %
                             i, self.deprocess_image(rgb * alpha))
        ...
        if FLAGS.dry_run or FLAGS.dry_run_inference:
            ...
            if FLAGS.transform_inverse_reg:
                ...
                inputs['jformattedInput'] = self.deprocess_image(
                    net_input_jitter[0])
            ...
            if FLAGS.dry_run:
                ...
                for i in range(FLAGS.num_psv_planes * 2):
                    ...
                    input['formattedInput'] = np.reshape(
                        input['formattedInput'], (320, 640, -1, 3))
                    write_image(dryrun_dir + "/formatInputReshaped_%s.png" %
                                i, input['formattedInput'][:, :, i, :])
                    #write_image(dryrun_dir + "/formatInput_%s.png" % i, input['formattedInput'][:,:,i*3:(i+1)*3])
                    if FLAGS.transform_inverse_reg:
                        write_image(dryrun_dir + "/formatJitteredInput_%s.png" %
                                    i, input['jformattedInput'][:, :, i*3:(i+1)*3])
                exit()
                ...
                with sv.managed_session(config=config) as sess:
                    ...
                    if FLAGS.transform_inverse_reg:
                        [input, rgba, jitter_rgba] = sess.run(
                            [inputs, rgba_layers, rgba_layers_jitter])
                    else:
                        [input, rgba] = sess.run([inputs, rgba_layers])

                for i in range(FLAGS.num_psv_planes * 2):
                    write_image(dryrun_dir + "/formatInput_%s.png" %
                                i, input['formattedInput'][:, :, i*3:(i+1)*3])
                    if FLAGS.transform_inverse_reg:
                        write_image(dryrun_dir + "/formatJitteredInput_%s.png" %
                                    i, input['jformattedInput'][:, :, i*3:(i+1)*3])
                ...
                for i in range(FLAGS.num_msi_planes):
                    ...
                    write_image(dryrun_dir + '/msi_alpha_%.2d.png' %
                                i, alpha_img)
                    write_image(dryrun_dir + '/msi_rgb_%.2d.png' % i, rgb_img)
                    if FLAGS.transform_inverse_reg:
                        jalpha_img = jitter_rgba[0, :, :, i, 3] * 255.0
                        jrgb_img = (
                            jitter_rgba[0, :, :, i, :3] + 1.) / 2. * 255
                        write_image(
                            dryrun_dir + '/jitter_msi_alpha_%.2d.png' % i, jalpha_img)
                        write_image(
                            dryrun_dir + '/jitter_msi_rgb_%.2d.png' % i, jrgb_img)

                # reconstruct graph to fit re-rendering in memory
                tf.reset_default_graph()
                tgt_pos = tf.constant(input["tgt_pose"])
                intrinsics_inv = tf.matrix_inverse(
                    tf.constant(intrinsics), name='intrinsics_inv')
                ...
                if FLAGS.input_type == 'ODS':
                    ...
                    if FLAGS.transform_inverse_reg:
                        ...
                        output_images["jitter_output_image"] = self.deprocess_image(self.msi_render_equirect_view(
                            tf.constant(jitter_rgba), jitter_pose, tgt_pos, msi_planes, intrinsics))[0]
                        output_images["jitter_output_depth"] = self.deprocess_image(self.msi_render_equirect_depth(
                            tf.constant(jitter_rgba), jitter_pose, tgt_pos, msi_planes, intrinsics))[0]
                        if 'src' in FLAGS.supervision:
                            output_images["jsrc_output_image"] = self.deprocess_image(self.msi_render_ods_view(
                                tf.constant(jitter_rgba), -1, jitter_pose, tgt_pos, msi_planes, intrinsics))[0]
                        if 'ref' in FLAGS.supervision:
                            output_images["jref_output_image"] = self.deprocess_image(self.msi_render_ods_view(
                                tf.constant(jitter_rgba), 1, jitter_pose, tgt_pos, msi_planes, intrinsics))[0]
                    if 'src' in FLAGS.supervision:
                        output_images["src_output_image"] = self.deprocess_image(self.msi_render_ods_view(
                            tf.constant(rgba), -1, tf.expand_dims(tf.eye(4), axis=0), tgt_pose, msi_planes, intrinsics))[0]
                    if 'ref' in FLAGS.supervision:
                        output_images["ref_output_image"] = self.deprocess_image(self.msi_render_ods_view(
                            tf.constant(rgba), 1, tf.expand_dims(tf.eye(4), axis=0), tgt_pose, msi_planes, intrinsics))[0]
                ...
                if FLAGS.input_type == 'ODS':
                    write_image(dryrun_dir + "/tgtp_rendered.png",
                                outputs["output_pimage"])
                    write_image(dryrun_dir + "/tgt_rendered.png",
                                outputs["output_image"])
                    write_image(dryrun_dir + "/depth_rendered.png",
                                outputs["output_depth"])
                    if FLAGS.transform_inverse_reg:
                        if 'tgt' in FLAGS.supervision:
                            write_image(
                                dryrun_dir + "/tgt_rendered_from_jitter.png", outputs["jitter_output_image"])
                            write_image(
                                dryrun_dir + "/depth_rendered_from_jitter.png", outputs["jitter_output_depth"])
                        if 'src' in FLAGS.supervision:
                            write_image(
                                dryrun_dir + "/src_rendered_from_jitter.png", outputs["jsrc_output_image"])
                        if 'ref' in FLAGS.supervision:
                            write_image(
                                dryrun_dir + "/ref_rendered_from_jitter.png", outputs["jref_output_image"])
                    if 'src' in FLAGS.supervision:
                        write_image(dryrun_dir + "/src_rendered.png",
                                    outputs["src_output_image"])
                    if 'ref' in FLAGS.supervision:
                        write_image(dryrun_dir + "/ref_rendered.png",
                                    outputs["ref_output_image"])
                elif FLAGS.input_type == 'PP':
                    write_image(dryrun_dir + "/tgt_rendered.png",
                                outputs["output_image"])
                exit()
    ...
    def train(self, train_op, checkpoint_dir, continue_train, summary_freq,
        ...
        saver = tf.train.Saver(
            [var for var in tf.trainable_variables()] + [global_step], max_to_keep=10)
        ...
        with sv.managed_session(config=config) as sess:
            ...
            if continue_train:
                ...
                if checkpoint is not None:
                    ...
                    tf.logging.info(
                        'Could not resume training from previous checkpoints')
            ...
            for step in range(1, max_steps):
                ...
                if step % save_latest_freq == 0:
                    ...
                    saver.save(sess, os.path.join(checkpoint_dir,
                                                  'model.latest'), global_step=global_step)
    ...
    def format_network_input(self, ref_image, src_image,
        ...
        if FLAGS.input_type != 'PRO2':
            ...
            for i in range(num_psv_source):
                curr_pose = tf.matmul(psv_src_poses[i:i+1, :, :], ref_pose_inv)
                curr_image = psv_src_images[:, :, :, i * 3:(i + 1) * 3]
                curr_psv = self.sweep_src(
                    curr_image, -1 if (i % 2) == 0 else 1, planes, curr_pose, intrinsics)
                net_input.append(curr_psv)
            net_input = tf.concat(net_input, axis=3)
            return net_input
        else:
            ...
            for i in range(2):
                curr_psv = self.sweep_src(
                    src_image, -1 if (i % 2) == 0 else 1, planes, src_pose, intrinsics)
                net_input.append(curr_psv)
            net_input = tf.concat(net_input, axis=3)
            return net_input
    ...
    def create_spherical_weights(self, epsilon=1.0e-12):
        ...
        spherical_attention = 1 / \
            abs(tf.cos(grid[1])-tf.cos(grid_shift[1])) * \
            abs(grid_shift[0] - grid[0])
    ...
    def sweep_ref(self, image, intrinsics):
        ...
        if FLAGS.jitter:
            ...
            pose = tf.tile(tf.expand_dims(
                tf.eye(4, dtype=tf.float32), 0), [batch, 1, 1])
        if FLAGS.input_type == 'ODS':
    ...
    def sweep_src(self, image, order, depths, pose, intrinsics):
        ...
        elif FLAGS.input_type == 'PRO2':
            return pj.pro2_sphere_sweep(image, order, depths, pose, intrinsics)
        else:
    ...
    def inv_depths(self, start_depth, end_depth, num_depths):
        ...
        for i in range(1, num_depths - 1):
            ...
            inv_depth = inv_start_depth + \
                (inv_end_depth - inv_start_depth) * fraction

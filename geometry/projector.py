...
def projective_forward_sphere(src_images, intrinsics, tgt_pose_rt, tgt_pos, depths):
    ...
    for i in range(n_batch):
        pixel_coords_batch = []
        pixels = spherical.intersect_sphere(
            tgt_pose_rt[i], tgt_pos[i], depths[:, i], n_layers, n_batch, width, height)
...
def projective_forward_sphere_to_perspective(src_images, intrinsics, tgt_pose_rt, tgt_pos, depths, viewing_window=3, tgt_height=320, tgt_width=640):
    ...
    for i in range(n_batch):
        pixel_coords_batch = []
        pixels = spherical.intersect_perspective(
            tgt_pose_rt[i], tgt_pos[i], depths[:, i], n_layers, n_batch, src_width, src_height, tgt_width, tgt_height, intrinsics)
        pixel_coords.append(pixels)
    pixel_coords = tf.stack(pixel_coords, axis=0)
    pixel_coords = tf.transpose(pixel_coords, [1, 0, 2, 3, 4])
...
def projective_forward_ods(src_images, order, intrinsics, jitter_pose, pos, depths):
    ...
    for i in range(n_batch):
        pixel_coords_batch = []
        pixels = spherical.intersect_ods(
            jitter_pose[i], pos[i], order, intrinsics, depths[:, i], n_layers, n_batch, width, height)
        pixel_coords.append(pixels)
    pixel_coords = tf.stack(pixel_coords, axis=0)
    pixel_coords = tf.transpose(pixel_coords, [1, 0, 2, 3, 4])
...
def gcn_sweep_one(image, order, depths, coord, pose, intrinsics, st_fun, backproj_fun, proj_fun):
    ...
    for i in range(batch):
        ...
        points = expand_along_depth(
            points, tf.convert_to_tensor(depths, tf.float32))
    ...
    all_resampled = tf.stack(all_resampled)
    resampled = tf.reshape(
        all_resampled, [batch, 1, n_vertex, channels * num_planes])
    return resampled
...
def pro2_sphere_sweep(image, order, depths, pose, intrinsics):
    return sweep_one(image, order, depths, pose, intrinsics,
                     spherical.lat_long_grid, spherical.backproject_spherical, spherical.project_pro2, True)
...
def over_composite_depth(rgbas):
    ...
    for i in range(0, len(rgbas)):
        ...
        if i == 0:
            ...
            output = (i / len(rgbas)) * alpha_image + \
                output * (1.0 - alpha_image)
...
def mesh_to_equirect(meshcolors, p2v):
    ...
    # 0-32 blending weights; 32-64 alpha; 64-67 bg
    num_vertex, channels = meshcolors.get_shape().as_list()
...
def plane_sweep(image, depths, pose, intrinsics):
    ...
    for depth in depths:
        ...
        warped_image = projective_inverse_warp(
            image, curr_depth, pose, intrinsics)
